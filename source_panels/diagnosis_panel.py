...
class DiagnosisConfigPanel(BaseSourceConfigPanel):
    ...
    def get_item_filtering_details(self) -> tuple:
        return "mimiciv_hosp.d_icd_diagnoses", "long_title", "icd_code", "筛选字段: long_title (mimiciv_hosp.d_icd_diagnoses)", None

    def get_value_column_for_aggregation(self) -> str | None:
        return None # 诊断通常不聚合数值

    def get_time_column_for_windowing(self) -> str | None:
        return None # diagnoses_icd 没有自己的时间戳，其时间上下文来自 admissions 表

    def get_aggregation_config_widget(self) -> QWidget | None:
        return None # 使用主Tab的通用事件输出UI

    def get_time_window_options(self) -> list | None:
        return None # 使用主Tab的通用事件时间窗口 (diag_event_time_window_options)
