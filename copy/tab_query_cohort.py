...
class CohortCreationWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # --- NEW: Add primary diagnosis for procedure-based cohorts ---
            ...
            if event_source_type_str == MODE_PROCEDURE_KEY:
                ...
                # Join to get the primary diagnosis (seq_num = 1) for the admission
                main_diag_join_sql = psql.SQL("""
                LEFT JOIN (
            ...
            create_target_table_sql = psql.SQL("""
                ...
                CREATE TABLE {target_ident} AS (
                 ...
                 {main_diag_join} -- Add the join for primary diagnosis here
                );
            """).format(
                ...
                main_diag_join=main_diag_join_sql # NEW
            )
            ...
            # --- NEW: Add index for primary_diag_code if it exists ---
            if event_source_type_str == MODE_PROCEDURE_KEY:
...
class QueryCohortTab(QWidget):
    ...
    def init_ui(self):
        ...
        controls_and_preview_layout.addWidget(self.condition_group)
    ...
    def on_mode_changed(self):
        ...
        if self.rb_mode_disease.isChecked():
            ...
            if hasattr(self, 'condition_group'): self.condition_group.set_search_field("long_title")
            if hasattr(self, 'query_btn'): self.query_btn.setText("查询疾病ICD")
            if hasattr(self, 'preview_btn'): self.preview_btn.setText("预览疾病查询SQL")
            if hasattr(self, 'admission_type_label'): self.admission_type_label.setText("选择疾病入院类型:")
            ...
            if hasattr(self, 'condition_group'): self.condition_group.set_search_field("long_title")
            if hasattr(self, 'query_btn'): self.query_btn.setText("查询手术/操作ICD")
            if hasattr(self, 'preview_btn'): self.preview_btn.setText("预览手术查询SQL")
            if hasattr(self, 'admission_type_label'): self.admission_type_label.setText("选择手术/操作入院类型:")
        ...
        if hasattr(self, 'query_btn'): # Ensure buttons exist before updating state
            self.update_button_states()
    ...
    def prepare_for_cohort_creation(self, starting=True):
        ...
        self.create_table_btn.setEnabled(is_enabled)
        self.query_btn.setEnabled(is_enabled)
        self.preview_btn.setEnabled(is_enabled)
    ...
    def update_button_states(self):
        ...
        self.query_btn.setEnabled(has_input and not is_worker_running)
        self.preview_btn.setEnabled(has_input and not is_worker_running)
        
        can_create_table = (has_input and 
                            self.result_table.rowCount() > 0 and 
                            self.last_query_condition_template and 
                            self.last_query_condition_template != "1=1" and 
                            not is_worker_running)
        self.create_table_btn.setEnabled(can_create_table)

        if not has_input and not is_worker_running:
            self.last_query_condition_template = None
            self.last_query_params = None
    ...
    def _build_query_parts(self):
        ...
        if condition_template:
            full_query = psql.SQL("{base} WHERE {condition}").format(base=base_query, condition=psql.SQL(condition_template))
        else:
    ...
    def preview_sql_action(self):
        ...
        if not db_params:
            try:
                dummy_conn = psycopg2.connect("dbname=dummy") # Create a dummy offline context
                query_template_for_mogrify = query_obj.as_string(dummy_conn) 
                dummy_conn.close()
            except: 
                query_template_for_mogrify = str(query_obj) # Fallback
        ...
        try:
            ...
            try:
                dummy_conn = psycopg2.connect("dbname=dummy")
                fallback_template = query_obj.as_string(dummy_conn)
                dummy_conn.close()
            except:
                fallback_template = str(query_obj)
            self.sql_preview.setText(f"SQL Template ({query_type_str} 查询):\n{fallback_template}\n\nParameters:\n{params}\n\n(生成预览时出错: {e})")
        finally:
    ...
    def execute_query(self):
        ...
        query_obj, params = self._build_query_parts()
        query_template_for_exec = query_obj
        ...
        if not has_meaningful_condition: self.create_table_btn.setEnabled(False)
        self.last_query_condition_template, self.last_query_params = self.condition_group.get_condition()
        if not self.last_query_condition_template and has_meaningful_condition: self.last_query_condition_template = "1=1" # Fallback if somehow empty
        self.preview_sql_action()
        conn = None
        try:
            conn = psycopg2.connect(**db_params)
            cur = conn.cursor()
            cur.execute(query_template_for_exec, params)
    ...
    def _generate_cohort_creation_sql_preview(self, target_table_name_str,
        ...
        try:
            ...
            base_event_select_preview_list = [
            ...
            if event_time_col_ident:
                base_event_select_preview_list.append(psql.SQL("e.{} AS qualifying_event_time").format(event_time_col_ident))
            
            base_event_select_sql_preview = psql.SQL("SELECT {select_cols} FROM {event_table} e JOIN {dict_table} dd ON e.{event_code_col} = dd.{dict_code_col} AND e.icd_version = dd.icd_version JOIN mimiciv_hosp.admissions adm ON e.hadm_id = adm.hadm_id WHERE ({condition_template_placeholder})").format(
                select_cols=psql.SQL(", ").join(base_event_select_preview_list), event_table=event_table_ident, dict_table=dict_table_ident,
                event_code_col=event_code_col_ident, dict_code_col=dict_code_col_ident, condition_template_placeholder=psql.SQL(condition_sql_template_str)
            )
            if admission_cohort_type == COHORT_TYPE_FIRST_EVENT_KEY:
                ...
                if event_time_col_ident and source_mode_details['source_type'] == MODE_PROCEDURE_KEY:
                    order_by_list_preview.append(psql.SQL("base.qualifying_event_time ASC NULLS LAST"))
                ...
                create_event_temp_sql_obj = psql.SQL("DROP TABLE IF EXISTS {temp_table_ident}; CREATE TEMPORARY TABLE {temp_table_ident} AS SELECT * FROM (SELECT base.*, ROW_NUMBER() OVER(PARTITION BY base.subject_id ORDER BY {order_by_clause}) AS admission_rank_for_event FROM ({base_select_preview_inner}) AS base) ranked_base WHERE ranked_base.admission_rank_for_event = 1;").format(
                    temp_table_ident=selected_event_ad_temp_ident, base_select_preview_inner=base_event_select_sql_preview, order_by_clause=psql.SQL(', ').join(order_by_list_preview)
                )
            elif admission_cohort_type == COHORT_TYPE_ALL_EVENTS_KEY:
                create_event_temp_sql_obj = psql.SQL("DROP TABLE IF EXISTS {temp_table_ident}; CREATE TEMPORARY TABLE {temp_table_ident} AS ({base_select_preview});").format(
                    temp_table_ident=selected_event_ad_temp_ident, base_select_preview=base_event_select_sql_preview
                )
            ...
            create_first_icu_sql_obj = psql.SQL("DROP TABLE IF EXISTS {temp_table_ident}; CREATE TEMPORARY TABLE {temp_table_ident} AS (SELECT * FROM (SELECT icu.subject_id, icu.hadm_id, icu.stay_id, icu.intime AS icu_intime, icu.outtime AS icu_outtime, EXTRACT(EPOCH FROM (icu.outtime - icu.intime)) / 3600.0 AS los_icu_hours, ROW_NUMBER() OVER (PARTITION BY icu.hadm_id ORDER BY icu.intime ASC, icu.stay_id ASC) AS icu_stay_rank_in_admission FROM mimiciv_icu.icustays icu WHERE EXISTS (SELECT 1 FROM {selected_event_temp_table} seat WHERE seat.hadm_id = icu.hadm_id)) sub WHERE icu_stay_rank_in_admission = 1);").format(
                temp_table_ident=first_icu_stays_temp_ident, selected_event_temp_table=selected_event_ad_temp_ident)
            readable_sql2 = create_first_icu_sql_obj.as_string(conn)
            target_select_list_preview = [
                psql.SQL("evt_ad.subject_id"), psql.SQL("evt_ad.hadm_id"), psql.SQL("evt_ad.admittime"), psql.SQL("adm.dischtime"),
                psql.SQL("icu.stay_id"), psql.SQL("icu.icu_intime"), psql.SQL("icu.icu_outtime"), psql.SQL("icu.los_icu_hours"),
                psql.SQL("evt_ad.qualifying_event_code"), psql.SQL("evt_ad.qualifying_event_icd_version"),
                # MODIFIED LINE:
                psql.SQL("CAST({source_literal} AS VARCHAR(20)) AS {col_alias}").format(
                    source_literal=psql.Literal(event_source_type_str),
                    col_alias=psql.Identifier("qualifying_event_source")
                ),
                psql.SQL("evt_ad.qualifying_event_title"), psql.SQL("evt_ad.qualifying_event_seq_num")
            ]
            if event_time_col_ident: target_select_list_preview.append(psql.SQL("evt_ad.qualifying_event_time"))
                        # --- NEW: Add primary diagnosis for procedure-based cohorts preview ---
            main_diag_join_sql_preview = psql.SQL("")
            if event_source_type_str == MODE_PROCEDURE_KEY:
                target_select_list_preview.extend([
                    psql.SQL("primary_dx.icd_code AS primary_diag_code"),
                    psql.SQL("primary_dx.icd_version AS primary_diag_icd_version"),
                    psql.SQL("primary_d_dx.long_title AS primary_diag_title")
                ])
                ...
                """)
            # --- END NEW ---
            create_target_sql_obj = psql.SQL("DROP TABLE IF EXISTS {target_ident}; CREATE TABLE {target_ident} AS (SELECT {select_cols} FROM {event_ad_temp_table} evt_ad JOIN mimiciv_hosp.admissions adm ON evt_ad.hadm_id = adm.hadm_id LEFT JOIN {icu_temp_table} icu ON evt_ad.hadm_id = icu.hadm_id {main_diag_join});").format(
                target_ident=target_ident, 
                select_cols=psql.SQL(', ').join(target_select_list_preview),
                event_ad_temp_table=selected_event_ad_temp_ident, 
                icu_temp_table=first_icu_stays_temp_ident,
                main_diag_join=main_diag_join_sql_preview # NEW
            )
            ...
            full_preview = (f"-- ===== Cohort Creation SQL Preview =====\n\n"
                            ...
                            f"-- Step 2: Create temporary table for first ICU stays --\n{readable_sql2}\n\n"
                            f"-- Step 3: Create final cohort table '{target_table_name_str}' --\n{readable_sql3}\n\n"
    ...
    def create_cohort_table_with_preview(self):
        ...
        if not success: QMessageBox.critical(self, "预览失败", "无法生成队列创建SQL的预览。\n请查看SQL预览区域的错误信息。"); return
        mode_text = "疾病ICD" if self.current_mode_key == MODE_DISEASE_KEY else "手术/操作ICD"
        reply = QMessageBox.question(self, '确认创建队列', f"将要创建基于 '{mode_text}' 的队列数据表 '{target_table_name_str}'.\n入院类型: {self.admission_type_combo.currentText()}.\n请检查SQL预览区域显示的语句。\n\n确定要继续吗?",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.No: self.sql_preview.append("\n\n-- 用户取消了队列创建操作。 --"); self.update_button_states(); return
        self.prepare_for_cohort_creation(True)
        self.cohort_worker = CohortCreationWorker(db_params, target_table_name_str, self.last_query_condition_template,
                                                self.last_query_params, selected_admission_type_key, current_source_mode_details)
        ...
        self.cohort_worker.finished.connect(self.cohort_worker_thread.quit); self.cohort_worker.error.connect(self.cohort_worker_thread.quit)
    ...
    def on_cohort_creation_finished(self, table_name, count):
        ...
        self.prepare_for_cohort_creation(False) # This now calls update_button_states
    ...
    def preview_created_cohort_table(self, schema_name, table_name):
        ...
        try:
            ...
            preview_query_str = "SELECT * FROM {} ORDER BY subject_id, hadm_id LIMIT 100;"
            preview_query = psql.SQL(preview_query_str).format(table_identifier)
