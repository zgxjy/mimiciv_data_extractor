            ...
            for i, (sql_obj_or_str, params_for_step) in enumerate(self.execution_steps):
                ...
                if isinstance(sql_obj_or_str, (pgsql.Composed, pgsql.SQL)):
                    try:
                        ...
                        if temp_conn_for_as_string != conn_merge and not temp_conn_for_as_string.closed : temp_conn_for_as_string.close()
...
class SpecialInfoDataExtractionTab(QWidget):
    ...
    def init_ui(self):
        ...
        # --- MODIFICATION: For Dynamic Field Hint ---
        condition_filter_outer_layout = QVBoxLayout() 

        self.search_field_hint_label = QLabel("当前筛选字段: label") 
        self.search_field_hint_label.setStyleSheet("font-style: italic; color: gray;")
        condition_filter_outer_layout.addWidget(self.search_field_hint_label)

        condition_filter_inner_layout = QHBoxLayout() 
        self.condition_widget = ConditionGroupWidget(is_root=True, search_field="label")
        condition_filter_inner_layout.addWidget(self.condition_widget)
        self.filter_items_btn = QPushButton("筛选项目"); self.filter_items_btn.clicked.connect(self.filter_source_items)
        condition_filter_inner_layout.addWidget(self.filter_items_btn, 0, Qt.AlignmentFlag.AlignTop)
        
        condition_filter_outer_layout.addLayout(condition_filter_inner_layout) 
        source_main_layout.addLayout(condition_filter_outer_layout) 
        # --- END MODIFICATION ---
        ...
        # --- MODIFICATION: Pass friendly name to update_source_config ---
        self.rb_lab.toggled.connect(lambda checked: self.update_source_config(checked, "labevents", "label", "itemid", "化验项目名 (label)"))
        self.rb_med.toggled.connect(lambda checked: self.update_source_config(checked, "prescriptions", "drug", "drug", "药物名 (drug)"))
        self.rb_proc.toggled.connect(lambda checked: self.update_source_config(checked, "procedures_icd", "long_title", "icd_code", "操作/手术名 (long_title)"))
        self.rb_diag.toggled.connect(lambda checked: self.update_source_config(checked, "diagnoses_icd", "long_title", "icd_code", "诊断名 (long_title)"))
        # --- END MODIFICATION ---
        ...
        logic_layout.addWidget(QLabel("新列基础名 (自动生成, 可修改):"), 0, 0)
        self.new_column_name_input = QLineEdit(); self.new_column_name_input.setPlaceholderText("自动生成或手动输入列基础名...")
        ...
        lab_logic_layout_outer = QVBoxLayout(self.lab_options_widget) 
        lab_logic_layout_outer.setContentsMargins(0,0,0,0)
        
        self.lab_agg_options_group = QGroupBox("提取方式 (可多选)")
        lab_agg_options_layout_inner = QGridLayout() 

        self.cb_lab_first = QCheckBox("首次 (First)"); self.cb_lab_last = QCheckBox("末次 (Last)")
        self.cb_lab_min = QCheckBox("最小值 (Min)");   self.cb_lab_max = QCheckBox("最大值 (Max)")
        self.cb_lab_mean = QCheckBox("平均值 (Mean)"); self.cb_lab_count = QCheckBox("计数 (Count)")
        self.lab_agg_checkboxes = [self.cb_lab_first, self.cb_lab_last, self.cb_lab_min, self.cb_lab_max, self.cb_lab_mean, self.cb_lab_count]

        lab_agg_options_layout_inner.addWidget(self.cb_lab_first, 0, 0); lab_agg_options_layout_inner.addWidget(self.cb_lab_last, 0, 1)
        lab_agg_options_layout_inner.addWidget(self.cb_lab_min, 1, 0);   lab_agg_options_layout_inner.addWidget(self.cb_lab_max, 1, 1)
        lab_agg_options_layout_inner.addWidget(self.cb_lab_mean, 2, 0);  lab_agg_options_layout_inner.addWidget(self.cb_lab_count, 2, 1)
        
        for cb in self.lab_agg_checkboxes:
            cb.stateChanged.connect(self.update_default_col_name)
            cb.stateChanged.connect(self.update_action_buttons_state)
        self.lab_agg_options_group.setLayout(lab_agg_options_layout_inner)
        lab_logic_layout_outer.addWidget(self.lab_agg_options_group) 
        ...
        event_logic_layout_outer = QVBoxLayout(self.event_options_widget)
        event_logic_layout_outer.setContentsMargins(0,0,0,0)

        self.event_output_options_group = QGroupBox("输出类型 (可多选)")
        event_output_options_layout_inner = QGridLayout()
        self.cb_event_exists = QCheckBox("是否存在 (Boolean)")
        self.cb_event_count = QCheckBox("发生次数 (Integer)")
        self.event_output_checkboxes = [self.cb_event_exists, self.cb_event_count]

        event_output_options_layout_inner.addWidget(self.cb_event_exists, 0,0)
        event_output_options_layout_inner.addWidget(self.cb_event_count, 0,1)

        for cb in self.event_output_checkboxes:
            cb.stateChanged.connect(self.update_default_col_name)
            cb.stateChanged.connect(self.update_action_buttons_state)
        self.event_output_options_group.setLayout(event_output_options_layout_inner)
        event_logic_layout_outer.addWidget(self.event_output_options_group)

        event_time_window_layout = QHBoxLayout()
        event_time_window_layout.addWidget(QLabel("时间窗口:"))
        ...
        event_time_window_layout.addWidget(self.event_time_window_combo)
        event_logic_layout_outer.addLayout(event_time_window_layout)
    ...
    def _get_current_source_config(self):
        ...
        elif checked_id == 4: return "mimiciv_hosp.diagnoses_icd", "mimiciv_hosp.d_icd_diagnoses", "long_title", "icd_code", None, None 
    ...
    def _generate_default_column_name(self):
        ...
        if self.rb_lab.isChecked():
            ...
            logic_map = {"是否存在 (Boolean)": "has", "发生次数 (Integer)": "count"}
            for cb in self.event_output_checkboxes:
                if cb.isChecked():
                    first_selected_method_text = cb.text()
                    break
        ...
        if first_selected_method_text: 
            ...
            parts.append("val" if self.rb_lab.isChecked() else "evt")
    ...
    def prepare_for_long_operation(self, starting=True):
        if starting:
            ...
            for cb_group_list in [self.lab_agg_checkboxes, self.event_output_checkboxes]:
                for cb_item in cb_group_list:
                    cb_item.setEnabled(False)
            self.lab_time_window_combo.setEnabled(False)
            self.event_time_window_combo.setEnabled(False)
            ...
            self.lab_time_window_combo.setEnabled(True)
            self.event_time_window_combo.setEnabled(True)
    ...
    # --- MODIFIED update_source_config ---
    def update_source_config(self, checked, source_table_name, name_col_for_cond_widget, id_col_for_data, friendly_search_field_name):
        if checked:
            self.condition_widget.set_search_field(name_col_for_cond_widget)
            self.search_field_hint_label.setText(f"当前筛选字段: {friendly_search_field_name}") # Update hint
    ...
    def update_action_buttons_state(self):
        ...
        any_agg_method_selected = False
        if self.rb_lab.isChecked():
            any_agg_method_selected = any(cb.isChecked() for cb in self.lab_agg_checkboxes)
        else: # Event
            any_agg_method_selected = any(cb.isChecked() for cb in self.event_output_checkboxes)

        can_act = (self.selected_cohort_table is not None and
                   len(self.item_list.selectedItems()) > 0 and
                   is_valid_col_name and
                   any_agg_method_selected) 
    ...
    def _build_merge_query(self, preview_limit=100, for_execution=False):
        ...
        if not selected_item_ids_values:
            return None, "未选择要合并的项目.", [], generated_column_details_for_preview
        ...
        if not source_event_table:
            return None, "无法确定数据来源表.", [], generated_column_details_for_preview
        ...
        from_join_clause_for_cte = pgsql.SQL( # Default for current hadm_id based events
            "FROM {event_table} {evt_alias} "
            "JOIN {cohort_table} {coh_alias} ON {evt_alias}.hadm_id = {coh_alias}.hadm_id"
        ).format(
        ...
        if is_lab:
            ...
            if actual_event_time_col_ident:
                ...
                return None, "化验事件需要有效的事件时间列来进行首次/末次提取。", params, generated_column_details_for_preview
            ...
            if is_diag: 
                ...
                else: # Diagnoses currently only selected for "prior" or implicitly current hadm_id
                    if actual_event_time_col_ident is None and source_event_table == "mimiciv_hosp.diagnoses_icd":
                        pass # No explicit time filter for current hadm_id diagnoses unless specific time col is used
                    elif actual_event_time_col_ident : # This case should ideally not be hit if UI restricts time window for Diag
                ...
                if "首次住院期间 (当前入院)" in time_option:
                    ...
                    if actual_event_time_col_ident:
                        time_filter_conditions.append(pgsql.SQL("{evt}.{time_col} BETWEEN {start_ts} AND {end_ts}").format(
                            ...
                            start_ts=cohort_admittime, end_ts=cohort_dischtime
                        ))
                    # If no actual_event_time_col (e.g. procedures_icd with null chartdate), no time filter applied for this window.
                elif "整个ICU期间 (当前入院)" in time_option:
                    # from_join_clause_for_cte uses the default hadm_id join
                    if actual_event_time_col_ident:
                        time_filter_conditions.append(pgsql.SQL("{evt}.{time_col} BETWEEN {start_ts} AND {end_ts}").format(
                            evt=event_alias, time_col=actual_event_time_col_ident,
                            start_ts=cohort_icu_intime, end_ts=cohort_icu_outtime
                        ))
                    ...
                    if source_event_table == "mimiciv_hosp.prescriptions":
                        ...
                        if actual_event_time_col_ident:
                            time_filter_conditions.append(pgsql.SQL("{evt}.{time_col} < {compare_ts}").format(
                                evt=event_alias, time_col=actual_event_time_col_ident, compare_ts=cohort_admittime
                            ))
                        else: # Prescriptions for prior history needs a time column
                             return None, "用药表进行既往史判断时缺少有效的事件时间列。", params, generated_column_details_for_preview
                    elif source_event_table == "mimiciv_hosp.procedures_icd":
                        from_join_clause_for_cte = pgsql.SQL( 
                            "FROM {event_table} {evt_alias} "
                            "JOIN mimiciv_hosp.admissions {adm_evt} ON {evt_alias}.hadm_id = {adm_evt}.hadm_id "
                            "JOIN {cohort_table} {coh_alias} ON {evt_alias}.subject_id = {coh_alias}.subject_id"
                        ...
                        time_filter_conditions.append(pgsql.SQL("{adm_evt}.admittime < {compare_ts}").format(
                            adm_evt=event_admission_alias, compare_ts=cohort_admittime
                        ))
                    else:
                        return None, f"“住院以前”逻辑未针对 {source_event_table} 实现。", params, generated_column_details_for_preview
                else: 
                    return None, f"未知的时间窗口选项: {time_option}", params, generated_column_details_for_preview
        
        all_where_conditions = item_id_conditions + time_filter_conditions
        if not item_id_conditions : 
             return None, "未能为FilteredEvents CTE构建有效的WHERE条件 (item ID条件缺失)。", params, generated_column_details_for_preview

        if not from_join_clause_for_cte: 
            return None, "内部错误：from_join_clause_for_cte 未被定义。", params, generated_column_details_for_preview
        ...
        filtered_events_cte_sql = pgsql.SQL(
            ...
            conditions=pgsql.SQL(' AND ').join(all_where_conditions) if all_where_conditions else pgsql.SQL("TRUE")
        )
        ...
        # generated_column_details_for_preview is already initialized at the top

        type_map_pgsql_to_str = { "NUMERIC": "Numeric", "INTEGER": "Integer", "BOOLEAN": "Boolean" } # Simplified map

        if is_lab:
            method_configs = [
                (self.cb_lab_first, "first", "(array_agg({val} ORDER BY {time} ASC NULLS LAST))[1]", pgsql.SQL("NUMERIC")),
                (self.cb_lab_last, "last", "(array_agg({val} ORDER BY {time} DESC NULLS LAST))[1]", pgsql.SQL("NUMERIC")),
                (self.cb_lab_min, "min", "MIN({val})", pgsql.SQL("NUMERIC")),
                (self.cb_lab_max, "max", "MAX({val})", pgsql.SQL("NUMERIC")),
                (self.cb_lab_mean, "mean", "AVG({val})", pgsql.SQL("NUMERIC")),
                (self.cb_lab_count, "count", "COUNT({val})", pgsql.SQL("INTEGER")), ]
            for cb, suffix, agg_template, col_type_sql_obj in method_configs:
                if cb.isChecked():
                    ...
                    if not is_valid_final_name:
                        return None, f"衍生列名错误 ({final_col_name_str}): {final_name_error}", params, generated_column_details_for_preview
                    selected_methods_details.append((final_col_name_str, pgsql.Identifier(final_col_name_str), agg_template, col_type_sql_obj))
                    col_type_str_raw = col_type_sql_obj.as_string(self.db_conn if self._connect_db() else pgsql.SQL("TEXT")) # Default to TEXT if no conn
            ...
            method_configs = [
                (self.cb_event_exists, "exists", "TRUE", pgsql.SQL("BOOLEAN")),
                (self.cb_event_count, "count", "COUNT(*)", pgsql.SQL("INTEGER")), ]
            for cb, suffix, agg_template, col_type_sql_obj in method_configs:
                if cb.isChecked():
                    final_col_name_str = f"{base_new_col_name_str}_{suffix}"
                    is_valid_final_name, final_name_error = self._validate_column_name(final_col_name_str)
                    if not is_valid_final_name:
                        return None, f"衍生列名错误 ({final_col_name_str}): {final_name_error}", params, generated_column_details_for_preview
                    selected_methods_details.append((final_col_name_str, pgsql.Identifier(final_col_name_str), agg_template, col_type_sql_obj))
                    col_type_str_raw = col_type_sql_obj.as_string(self.db_conn if self._connect_db() else pgsql.SQL("TEXT"))
                    generated_column_details_for_preview.append((final_col_name_str, type_map_pgsql_to_str.get(col_type_str_raw.upper(), col_type_str_raw)))

        if not selected_methods_details:
            return None, "请至少选择一种提取方式或输出类型。", params, generated_column_details_for_preview
        ...
        for _, final_col_ident, agg_template_str, _ in selected_methods_details:
            sql_expr = None
            if is_lab: 
                if "{time}" in agg_template_str and not actual_event_time_col_ident:
                    return None, f"提取方式 '{agg_template_str}' 需要事件时间列，但当前数据源未提供或未配置。", params, generated_column_details_for_preview
                
                if "{val}" in agg_template_str and "{time}" in agg_template_str:
        ...
        if not aggregated_columns_sql_list:
            return None, "未能构建聚合列。", params, generated_column_details_for_preview
        ...
        if for_execution:
            ...
            preview_sql = pgsql.SQL(
                "WITH MergeData AS ({data_gen_query}) "
                "SELECT {select_cols_list} "
                "FROM {target_table} {coh_alias} "
                "LEFT JOIN MergeData {md_alias} ON {coh_alias}.hadm_id = {md_alias}.{hadm_col_in_temp} "
                "ORDER BY {coh_alias}.subject_id, {coh_alias}.hadm_id LIMIT {limit};"
            ).format(
            ...
            return preview_sql, None, params, generated_column_details_for_preview
    
    # --- MODIFIED execute_merge ---
    def execute_merge(self):
        ...
        base_new_col_name_str = self.new_column_name_input.text().strip()
        is_valid_base_name, name_error = self._validate_column_name(base_new_col_name_str)
        if not is_valid_base_name: QMessageBox.warning(self, "基础列名无效", name_error); return
        
        any_method_selected = False
        if self.rb_lab.isChecked(): any_method_selected = any(cb.isChecked() for cb in self.lab_agg_checkboxes)
        else: any_method_selected = any(cb.isChecked() for cb in self.event_output_checkboxes)
        if not any_method_selected: QMessageBox.warning(self, "选择不完整", "请至少选择一种提取方式或输出类型。"); return

        # Get column details for the confirmation dialog from _build_merge_query
        # The 4th element is generated_column_details_for_preview
        # We call _build_merge_query once here to get all necessary info for dialog and execution
        # If _build_merge_query itself is very expensive, this could be optimized later
        # by having a lighter version just for col details, but for now this is safer for consistency.
        build_result = self._build_merge_query(for_execution=True)
        if build_result is None or len(build_result) < 4 : # Should not happen if errors are handled inside
             QMessageBox.critical(self, "内部错误", "构建查询时未能返回预期结果。"); return

        execution_steps_list, signal_type, new_cols_desc_for_worker, column_details_for_dialog = build_result

        if signal_type != "execution_list" : # An error message was returned instead of "execution_list"
            error_msg_from_build = signal_type if isinstance(signal_type, str) else "未能生成执行SQL步骤的详细信息。"
            QMessageBox.critical(self, "合并准备失败", f"无法构建SQL: {error_msg_from_build}"); return
        
        if not column_details_for_dialog:
            QMessageBox.critical(self, "无法继续", "未能生成列详情进行预览。"); return

        column_preview_message = f"确定要向表 '{self.selected_cohort_table}' 中添加/更新以下列吗？\n"
        for name, type_str in column_details_for_dialog:
            column_preview_message += f"\n - {name} (类型: {type_str})" # Display name and type
        ...
        # execution_steps_list and new_cols_desc_for_worker are already available from the build_result above.
        db_params = self.get_db_params()
        ...
        # --- MODIFIED Signal Connections for thread safety ---
        self.worker_thread.started.connect(self.merge_worker.run)
        
        self.merge_worker.finished.connect(self.on_merge_worker_finished_actions) # Renamed slot
        ...
        self.worker_thread.finished.connect(self.trigger_preview_after_thread_finish) # New slot for preview
    ...
    # --- NEW SLOT for actions after merge_worker.finished ---
    @Slot()
    def on_merge_worker_finished_actions(self):
        ...
        # request_preview_signal is now emitted by trigger_preview_after_thread_finish
    # --- END NEW SLOT ---

    # --- NEW SLOT connected to worker_thread.finished ---
    @Slot()
    def trigger_preview_after_thread_finish(self):
        if self.selected_cohort_table: # Check if table is still selected
            print(f"Worker thread finished. Requesting preview for: {self.selected_cohort_table}")
    ...
    def preview_merge_data(self):
        ...
        # The 4th element is generated_column_details_for_preview, which is ignored by this preview method
    ...
    def on_merge_finished(self):
        # This slot is now only connected to merge_worker.finished to trigger worker_thread.quit
        # The actual UI updates are in on_merge_worker_finished_actions
        pass
        
    # on_merge_error, closeEvent remain the same
    @Slot(str)
    def on_merge_error(self, error_message): # Original on_merge_error content
        self.update_execution_log(f"合并失败: {error_message}")
        if "操作已取消" not in error_message:
             QMessageBox.critical(self, "合并失败", f"执行合并SQL失败: {error_message}")
        else: QMessageBox.information(self, "操作取消", "数据合并操作已取消。")
        self.prepare_for_long_operation(False)
        if self.worker_thread and not self.worker_thread.isFinished(): # Defensive quit
            self.worker_thread.quit()
