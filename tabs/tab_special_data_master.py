...
from PySide6.QtCore import Qt, Signal, Slot, QObject, QThread, QTimer # 新增 QTimer
...
# (MergeSQLWorker 类代码保持不变 - 从您之前的代码复制)
...
class SpecialDataMasterTab(QWidget):
    ...
    def __init__(self, get_db_params_func, parent=None):
        ...
        self.init_ui()
        # 确保在UI完全构建后，通过setChecked来触发初始的面板更新流程
    ...
    def _update_active_panel(self, force_col_name_update=False):
        ...
        if active_panel:
            ...
            # 先 populate，确保子组件（如TimeWindowSelectorWidget）已初始化并可能发出初始信号
            if hasattr(active_panel, 'populate_panel_if_needed'):
                active_panel.populate_panel_if_needed()
            
            # QApplication.processEvents() # 允许 populate_panel_if_needed 触发的信号先处理
            # 使用 QTimer.singleShot 确保 populate_panel_if_needed 的信号处理完成后再执行后续逻辑
            # 这有助于解决问题1，确保按钮状态是基于最新的、完全初始化的面板配置。
    ...
    def _finish_update_active_panel(self, active_panel: BaseSourceConfigPanel, force_col_name_update: bool):
        """_update_active_panel 的后半部分，在 populate 完成后（通过QTimer延迟）调用。"""
        details = ("-", "-", "-", "配置筛选条件。", "-") 
    ...
    def _on_panel_config_changed(self):
        # 如果面板的配置发生变化（例如时间窗口、选定项目、聚合类型），
        # 我们应该允许默认列名重新生成并更新UI。
        # 重置此标志可确保 _generate_and_set_default_col_name
        # 在生成的名称与当前文本不同时更新 QLineEdit，
        # 即使 force_update 为 False。
        self.user_manually_edited_col_name = False

        # 此处 force_update=False 是可以的，因为 user_manually_edited_col_name 现在是 False。
        # 如果新生成的名称与 QLineEdit 中当前的内容不同，
        # _generate_and_set_default_col_name 将更新文本字段。
    ...
    def _generate_and_set_default_col_name(self, force_update=False):
        ...
        if not found_logic and active_panel: 
            if hasattr(active_panel, 'value_agg_widget') and active_panel.value_agg_widget:
                # Fallback for older widget direct access if needed, but panel_config should be primary
        ...
        if primary_label_from_config:
            ...
            # Try to get a more descriptive name from the item_list if possible
            # This part assumes the first selected item is representative for the name
            item_text_for_name = str(selected_ids_from_config[0]) # Fallback to ID
            if active_panel and hasattr(active_panel, 'item_list') and active_panel.item_list and active_panel.item_list.selectedItems():
                try:
                    # Prefer user_data[1] which is intended for column naming
                    ...
                    # Fallback if accessing item_list fails for some reason
                    pass # item_text_for_name remains the ID
        ...
        if time_window_text_from_config:
            ...
            if current_time_text_old:
                # This fallback might be less accurate if panel_config isn't populated yet
        ...
        final_name = default_name if default_name and default_name.strip() else "new_col" # 确保 default_name 非空且非纯空格
        ...
        if force_update or not self.user_manually_edited_col_name:
            ...
            if current_text != final_name:
                ...
                # 如果强制更新，或者是因为非手动编辑而更新了文本，
                # 并且新名称不是"new_col"（表示至少有一些有意义的部分生成了），
                # 那么可以认为这不是用户手动编辑的结果。
                if force_update: #  and final_name != "new_col":
                     self.user_manually_edited_col_name = False
            elif force_update and current_text == final_name: # 如果强制更新，且名字没变，也要重置flag
                self.user_manually_edited_col_name = False
                
    def _are_configs_valid_for_action(self) -> bool:
        if not self.selected_cohort_table: return False
        col_name_text = self.new_column_name_input.text().strip() # 在这里定义
        is_valid_col_name, name_err = validate_column_name(col_name_text) # 在这里定义
        ...
        # ... (后续的 selected_ids, agg_methods, evt_outputs 检查) ...
        agg_methods = panel_config.get("aggregation_methods")
        evt_outputs = panel_config.get("event_outputs")
    ...
    def on_db_connected(self):
        ...
        # _update_active_panel 会处理后续的列名和按钮状态更新
        # QTimer.singleShot(0, lambda: self._update_active_panel(force_col_name_update=True))
        # on_db_connected 之后，refresh_cohort_tables 会调用 on_cohort_table_selected，
        # on_cohort_table_selected 会调用 update_master_action_buttons_state。
        # _on_source_type_changed（如果初始setChecked触发）也会调用 _update_active_panel。
        # 避免在这里直接调用 _update_active_panel 以防止潜在的多重调用。
        # 按钮状态由 on_cohort_table_selected 和 _on_source_type_changed 驱动更新。


    def refresh_cohort_tables(self):
        # ... (与上一版本一致，确保 on_cohort_table_selected 在末尾被调用)
        ...
        if not db_params:
            ...
            self.on_cohort_table_selected(self.table_combo.currentIndex()) # 确保状态更新
            return
        try:
    ...
    def execute_merge(self):
        ...
        temp_conn_for_display = None
        readable_sql_steps = []
        try:
            if db_params: # 尝试为格式化创建一个临时连接
                temp_conn_for_display = psycopg2.connect(**db_params)
    ...
    def preview_merge_data(self):
        ...
        readable_sql_for_display = "-- 生成预览SQL时出错 --" 
        try:
            ...
            df = pd.read_sql_query(sql_string_for_pandas, conn_for_preview, params=params_list if params_list else None)
        ...
        if not conn or conn.closed:
            ...
            try:
                if params_list and '%s' in sql_template:
                    formatted_params = tuple(f"'{p}'" if isinstance(p, str) else p for p in params_list)
                    return sql_template % formatted_params
                return f"{sql_template} -- Params: {params_list if params_list else 'None'}"
            except TypeError: 
                return f"{sql_template} -- Params (formatting error): {params_list}"
        try:
